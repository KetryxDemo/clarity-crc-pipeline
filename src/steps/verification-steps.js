...
When('the ingestion module normalises it', function () {
  this.ingested = ingest(this.manifest, this.hgbFile);
});
...
Then('the specimen record carries the total DNA input and the conversion control status', function () {
  assert.equal(this.ingested.record.totalDnaInputNg, 120);
  assert.equal(this.ingested.record.conversionControl, 'pass');
});
...
When('the haemoglobin module validates the input', function () {
  this.hgbResult = validateHaemoglobin(this.hgbInput ?? this.specimen.haemoglobin);
});

Then('the specimen is scorable', function () {
...
Then('the specimen is not scorable', function () {
  assert.equal(this.hgbResult.scorable, false);
});

Then('the reason records a missing haemoglobin measurement', function () {
...
Then('the reason records that DNA input was not reported', function () {
  assert.match(this.adequacy.reason, /not reported/);
});

Then('the reason records a failed conversion control', function () {
...
When('the identity module verifies the accession chain', function () {
  this.identity = verifyIdentity(this.specimen.marker, this.specimen.haemoglobin);
});
...
When('the report module renders the report', function () {
  this.report = generateReport(this.reportFor, 'ACC-9004', {});
});
...
Given('a scored specimen and an available audit store', function () {
  ...
  this.scored = { accession: 'ACC-9005', result: 'POSITIVE', score: 0.61,
...
Given('a scored specimen and an unavailable audit store', function () {
  ...
  this.scored = { accession: 'ACC-9006', result: 'NEGATIVE', score: 0.11,
                  thresholdApplied: CONFIG.positivityThreshold };
});
...
Then('release is withheld', function () {
